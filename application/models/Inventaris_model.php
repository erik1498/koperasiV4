...
class inventaris_model extends CI_model{
    ...
    {
        $return = [];
        $data = $this->db->get_where('tbl_inventaris', ['waktu >='=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function getinventaris($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_inventaris');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        $sisa = str_replace(',','', $this->lksb_model->getLKSB(0)['Neraca']['Aktiva_tetap_2']['inventaris']);
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
            $jumlah[$i]['kredit'] = 0;
    ...
    public function simpaninventaris($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_inventaris' => is_null($edit) ? '' : $data['id'],
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_inventaris', $simpan);
            }else{
                return $this->db->replace('tbl_inventaris', $simpan);
            }
        }else{
            return true;
        }
    ...
    {
        $row = $this->db->get_where('tbl_inventaris', ['id_inventaris' => $id])->row_array();
        ...
        return $this->db->delete('tbl_inventaris', ['id_inventaris' => $id]);
    }
}