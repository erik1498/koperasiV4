...
class titipan_konsumsi_model extends CI_model{
    public function getCatatan($bulan, $tahun)
    {
        $return = [];
        $data = $this->db->get_where('tbl_titipan_konsumsi',['waktu >='=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
        ...
        // $hasil = $this->db->get_where('tbl_riwayat_simpanan', ['waktu >= ' => $tahun.'-'.$bulan.'-01', 'waktu <=' => $tahun.'-'.$bulan.'-31', 'jenis' => 'uang_konsumsi'])->result_array();
        ...
        //     'keterangan' => 'Akumulasi Uang Konsumsi',
        //     'jumlah' => $jum,
        //     'jenis' => 0
        // ];
        return $return;
    ...
    public function gettitipan_konsumsi($bulan, $tahun)
    {
    ...
    {
        // $this->db->from('tbl_titipan_konsumsi');
        // $this->db->where('waktu >=', $tahun.'-01-01');
        // $this->db->where('waktu <=', $tahun.'-12-31');
        // $this->db->order_by('waktu', 'ASC');
        // $hasil = $this->db->get()->result_array();
        $sisa = str_replace(',','', $this->lksb_model->getLKSB(0)['Modal']['titipan_konsumsi']);
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
    ...
    public function simpantitipan_konsumsi($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_titipan_konsumsi' => is_null($edit) ? '' : $data['id'],
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_titipan_konsumsi', $simpan);
            }else{
                return $this->db->replace('tbl_titipan_konsumsi', $simpan);
            }
        }else{
            return true;
        }
    ...
    public function hapustitipan_konsumsi($id)
    {

        $row = $this->db->get_where('tbl_titipan_konsumsi', ['id_titipan_konsumsi' => $id])->row_array();
        ...
        return $this->db->delete('tbl_titipan_konsumsi', ['id_titipan_konsumsi' => $id]);
    }
}