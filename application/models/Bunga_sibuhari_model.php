...
class bunga_sibuhari_model extends CI_model{
    public function getCatatan($bulan, $tahun)
    {
        $return = [];
        $data = $this->db->get_where('tbl_bunga_sibuhari',['waktu >='=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function getbunga_sibuhari($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_bunga_sibuhari');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        // $sisa = str_replace(',', '', $this->lksb_model->getLKSB(0)['Laporan_SHU']['pendapatan']['bunga_sibuhari_swastisari']);
        $sisa = 0;
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
    ...
    public function simpanbunga_sibuhari($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_bunga_sibuhari' => is_null($edit) ? '' : $data['id'],
                'waktu' => _inputWaktu($data['tahun'], $data['bulan'], $data['tanggal']),
                'jumlah' => str_replace(',','', $data['jumlah']),
                'jenis' => $data['jenis']
            ];
            if (is_null($edit)) {
                return $this->db->insert('tbl_bunga_sibuhari', $simpan);
            }else{
                return $this->db->replace('tbl_bunga_sibuhari', $simpan);
            }
        }else{
            return true;
        }
    ...
    public function hapusbunga_sibuhari($id)
    {
        $row = $this->db->get_where('tbl_bunga_sibuhari', ['id_bunga_sibuhari' => $id])->row_array();
        ...
        return $this->db->delete('tbl_bunga_sibuhari', ['id_bunga_sibuhari' => $id]);
    }
}