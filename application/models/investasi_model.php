...
class investasi_model extends CI_model{
    ...
    {
        $return = [];
        $data = $this->db->get_where('tbl_investasi',['waktu >=' => ($tahun - 1).'-12-31', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function getinvestasi($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_investasi');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        $sisa = str_replace(',','', $this->lksb_model->getLKSB(0)['Neraca']['Aktiva_lancar_1']['investasi']);
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
            $jumlah[$i]['kredit'] = 0;
    ...
    public function simpaninvestasi($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_investasi' => is_null($edit) ? '' : $data['id'],
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_investasi', $simpan);
            }else{
                return $this->db->replace('tbl_investasi', $simpan);
            }
        }else{
            return true;
        }
    ...
    public function hapusinvestasi($id)
    {
        $row = $this->db->get_where('tbl_investasi', ['id_investasi' => $id])->row_array();
        ...
        return $this->db->delete('tbl_investasi', ['id_investasi' => $id]);
    }
}