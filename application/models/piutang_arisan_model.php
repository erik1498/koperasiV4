...
class piutang_arisan_model extends CI_model{
    public function getCatatan($bulan, $tahun)
    {
        $return = [];
        $data = $this->db->get_where('tbl_piutang_arisan',['waktu >='=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function getpiutang_arisan($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_piutang_arisan');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        $sisa = floatval(str_replace(',','', $this->lksb_model->getLKSB(0)['Neraca']['Aktiva_lancar_1']['piutang_arisan']));
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
            $jumlah[$i]['kredit'] = 0;
    ...
    public function simpanpiutang_arisan($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_piutang_arisan' => is_null($edit) ? '' : $data['id'],
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_piutang_arisan', $simpan);
            }else{
                return $this->db->replace('tbl_piutang_arisan', $simpan);
            }
        }else{
            return true;
        }
    ...
    public function hapuspiutang_arisan($id)
    {
        $row = $this->db->get_where('tbl_piutang_arisan', ['id_piutang_arisan' => $id])->row_array();
        ...
        return $this->db->delete('tbl_piutang_arisan', ['id_piutang_arisan' => $id]);
    }
}