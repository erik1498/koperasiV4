...
class persediaan_model extends CI_model{
    ...
    {
        $return = [];
        $data = $this->db->get_where('tbl_persediaan',['waktu >='=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function getpersediaan($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_persediaan');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        $sisa = str_replace(',','', $this->lksb_model->getLKSB(0)['Neraca']['Aktiva_lancar_1']['persediaan_barang']);
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
            $jumlah[$i]['kredit'] = 0;
    ...
    public function simpanpersediaan($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_persediaan' => is_null($edit) ? '' : $data['id'],
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_persediaan', $simpan);
            }else{
                return $this->db->replace('tbl_persediaan', $simpan);
            }
        }else{
            return true;
        }
    ...
    {
        $row = $this->db->get_where('tbl_persediaan', ['id_persediaan' => $id])->row_array();
        ...
        return $this->db->delete('tbl_persediaan', ['id_persediaan' => $id]);
    }
}