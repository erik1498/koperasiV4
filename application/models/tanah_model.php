...
class tanah_model extends CI_model{
    ...
    {
        $return = [];
        $data = $this->db->get_where('tbl_tanah',['waktu >'=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function gettanah($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_tanah');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        $sisa = str_replace(',','', $this->lksb_model->getLKSB(0)['Neraca']['Aktiva_tetap_2']['tanah']);
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
            $jumlah[$i]['kredit'] = 0;
    ...
    public function simpantanah($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_tanah', $simpan);
            }else{
                return $this->db->replace('tbl_tanah', $simpan);
            }
        }else{
            return true;
        }
    ...
    public function hapustanah($id)
    {
        $row = $this->db->get_where('tbl_tanah', ['id_tanah' => $id])->row_array();
        ...
        return $this->db->delete('tbl_tanah', ['id_tanah' => $id]);
    }
}