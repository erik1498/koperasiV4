...
class hibah_donasi_model extends CI_model{
    public function getCatatan($bulan, $tahun)
    {
        $return = [];
        $data = $this->db->get_where('tbl_hibah_donasi',['waktu >='=> $tahun.'-01-01', 'waktu <=' => $tahun.'-12-31'])->result_array();
        foreach ($data as $d) {
    ...
    public function gethibah_donasi($bulan, $tahun)
    {
    ...
    {
        $this->db->from('tbl_hibah_donasi');
        $this->db->where('waktu >=', $tahun.'-01-01');
        $this->db->where('waktu <=', $tahun.'-12-31');
        $this->db->order_by('waktu', 'ASC');
        $hasil = $this->db->get()->result_array();
        $sisa = str_replace(',','', $this->lksb_model->getLKSB(0)['Modal']['hibah']);
        $jumlah = [];
        for ($i=1; $i <= 12; $i++) { 
            $jumlah[$i]['debit'] = 0;
            $jumlah[$i]['kredit'] = 0;
    ...
    public function simpanhibah_donasi($data, $edit = null)
    {
        ...
        if (str_replace(',','', $data['jumlah']) != "") {
            $simpan = [
                'id_hibah_donasi' => is_null($edit) ? '' : $data['id'],
            ...
            if (is_null($edit)) {
                return $this->db->insert('tbl_hibah_donasi', $simpan);
            }else{
                return $this->db->replace('tbl_hibah_donasi', $simpan);
            }
        }else{
            return true;
        }
    ...
    public function hapushibah_donasi($id)
    {
        $row = $this->db->get_where('tbl_hibah_donasi', ['id_hibah_donasi' => $id])->row_array();
        ...
        return $this->db->delete('tbl_hibah_donasi', ['id_hibah_donasi' => $id]);
    }
}