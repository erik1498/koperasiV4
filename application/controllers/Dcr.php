...
class Dcr extends CI_Controller{
    ...
    {
        $data['page'] = 'transaksi_dcr';
        $this->load->view('templates/header');
        $this->load->view('ADMIN/dcr/transaksi_dcr', $data);
    ...
    public function getCatatan()
    {
        ...
        $catatan = $this->dcr_model->getCatatan(intval($bulan), $this->session->userdata('tahun'));
        $akumulasi = $this->dcr_model->akumulasi($this->session->userdata('tahun'));
        echo json_encode(
            [
                'catatan' => $catatan,
                'akumulasi' => $akumulasi
    ...
    public function simpandcr($edit = null)
    {
        $data = $this->input->post();
        echo json_encode($this->dcr_model->simpandcr($data, $edit));
    }

    public function hapusdcr()
    {
        $id = $this->input->post('id');
        echo json_encode($this->dcr_model->hapusdcr($id));
    }
}