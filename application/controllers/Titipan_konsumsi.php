...
class Titipan_konsumsi extends CI_Controller{
    ...
    {
        $data['page'] = 'transaksi_titipan_konsumsi';
        $this->load->view('templates/header');
        $this->load->view('ADMIN/titipan_konsumsi/transaksi_titipan_konsumsi', $data);
    ...
    public function getCatatan()
    {
        ...
        $catatan = $this->titipan_konsumsi_model->getCatatan(intval($bulan), $this->session->userdata('tahun'));
        $akumulasi = $this->titipan_konsumsi_model->akumulasi($this->session->userdata('tahun'));
        echo json_encode(
            [
                'catatan' => $catatan,
                'akumulasi' => $akumulasi
    ...
    public function simpantitipan_konsumsi($edit = null)
    {
        $data = $this->input->post();
        echo json_encode($this->titipan_konsumsi_model->simpantitipan_konsumsi($data, $edit));
    }

    public function hapustitipan_konsumsi()
    {
        $id = $this->input->post('id');
        echo json_encode($this->titipan_konsumsi_model->hapustitipan_konsumsi($id));
    }
}